...
CREATE TABLE anomalies AS SELECT user_from, COUNT(user_from) as total_txns, sum(amount) as total_amount FROM transactions_stream WINDOW HOPPING (SIZE 10 SECONDS, ADVANCE BY 1 SECONDS) GROUP BY user_from HAVING COUNT(*) > 3;